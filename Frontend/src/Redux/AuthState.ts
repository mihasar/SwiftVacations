...
export interface AuthAction {
    type: AuthActionType;
    payload?: string; // the string is because we're getting a token, the ? is because sends nothing.
}
...
export function authReducer(currentState = new AuthState(), action: AuthAction): AuthState {
    ...
    switch (action.type) {
        ...
        case AuthActionType.Login:
            ...
            localStorage.setItem("token", newState.token); // Save token to storage for persisting after refresh
            break;
